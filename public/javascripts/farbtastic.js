...
jQuery._farbtastic = function (container, callback) {
  ...
  fb.setColor = function (color) {
    //MVR - test to make sure input is valid
    var pattern = /^#([0-9abcdefABCDEF]{3}|[0-9abcdefABCDEF]{6})$/;
    if (!pattern.test(color)) {
      //MVR - update the display and not the linked value
      //TODO: there is a bug with updating the display when #ffffff is input
      fb.color = "#ffffff";
      fb.rgb = [0, 0, 0];
      ...
      // Reset background/foreground color
  ...
  fb.updateDisplay = function () {
    ...
    if (typeof fb.callback == 'object') {
      ...
      jQuery(fb.callback).each(function() {
        //MVR - changed to update value if not set
