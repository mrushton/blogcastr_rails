class Users::SubscriptionsController < ApplicationController
  def index
    ...
    respond_to do |format|
        ...
        format.xml {render :xml => @user.subscriptions.to_xml(:only => [:id, :name], :include => :subscribed_to)}
        format.json {render :json => @user.subscriptions.to_json(:only => [:id, :name], :include => :subscribed_to)}
    end
  end
