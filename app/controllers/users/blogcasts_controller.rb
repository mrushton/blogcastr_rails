class Users::BlogcastsController < ApplicationController
  ...
  def index
    ...
    if @id.blank?
      @paginated_blogcasts = Blogcast.paginate_by_sql(["SELECT * FROM blogcasts WHERE user_id = ?", @profile_user.id], :page => @page, :per_page => 10)
      @id = Blogcast.maximum(:id, :conditions => ["user_id = ?", @profile_user.id])
    else
      @paginated_blogcasts = Blogcast.paginate_by_sql(["SELECT * FROM blogcasts WHERE user_id = ? AND id <= ?", @profile_user.id, @id], :page => @page, :per_page => 10, :total_entries => @num_blogcasts)
    end
